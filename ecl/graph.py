...
class Graph:
    ...
    def create_from_log(self, directory) -> None:
        ...
        utterances = [utterance for utterance in utterances if
                      "flask app.py" not in utterance and f"{BASE_URL} Usage_Info" not in utterance]
    ...
    def _create_instruction_and_roles_from_log(self, directory) -> None:
        ...
        for match in matches:
            ...
            # print()
        utterances = [utterance for utterance in utterances if
                      "Chief Technology Officer: **[Start Chat]**" in utterance or "Code Reviewer: **[Start Chat]**" in utterance or "Software Test Engineer: **[Start Chat]**" in utterance]
        ...
        for utterance in utterances:
            ...
            if "Chief Technology Officer: **[Start Chat]**".lower() in utterance:
                ...
                if "Test Pass!".lower() in utterance:
                    ...
                    instruction = utterance.split("Error Summary of Test Reports:".lower())[-1].split(
                        "Note that each file must strictly follow a markdown code block format".lower())[0]
            else:
                assert False
    ...
    def print(self):
        ...
        for edge in self.edges:
            output += "{}: {} -> {} ({}: {})\n".format(edge.edgeId, edge.sourceMID, edge.targetMID, edge.role,
                                                       edge.instruction[:60])
        output += "*" * 50 + " Graph " + "*" * 50
        log_and_print_online(output)
